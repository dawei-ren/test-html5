...
const tooltip = new G6.Tooltip({
  ...
  getContent(e) {
    ...
    outDiv.style.maxWidth = '400px';
    outDiv.innerHTML = `
      <ul>
        <li>commandName:${e.item.getModel().label}</li>
        <li>rawArg:${e.item.getModel().rawArg}</li>
      </ul>`
    return outDiv
  },
  itemTypes: ['node']
...
$('#to-json').click(() => {
  ...
  graph.getNodes().forEach(each => {
    myObject.nodes.push({
      "nodeId": each.getModel().id,
      "node": {
        "commandName": each.getModel().label,
        "jarUrl": null,
        "rawArg": each.getModel().rawArg
      }
    })
  })
  ...
  onClick(ev) {
    ...
    if (this.addingEdge && this.edge) {
      graph.updateItem(this.edge, {
        target: model.id,
        // label: this.edge.get("model")['label'],
        label: this.edge.getModel().label + model.id,
